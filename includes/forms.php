...
function rotify_qa_form_shortcode() {
    ...
    if ($require_login && !is_user_logged_in()) {
        return '<p class="rotify-error rotify-login-error">برای ارسال پرسش باید وارد شوید.</p>';
    }
    ...
    if (isset($_POST['qa_submit']) && isset($_POST['rotify_qa_nonce']) && wp_verify_nonce($_POST['rotify_qa_nonce'], 'rotify_qa_submit')) {
        ...
        if ($post_id) {
            ...
            echo '<p class="rotify-success rotify-submit-success">سوال شما با موفقیت ارسال شد و در انتظار تأیید است.</p>';
        }
    }
    return ob_get_clean();
}
...
function rotify_comment_form_shortcode() {
    ...
    if (is_user_logged_in() && $require_purchase) {
        $user_has_purchased = wc_customer_bought_product('', get_current_user_id(), $product_id);
    }
    ...
    if (isset($_POST['comment_submit']) && isset($_POST['rotify_comment_nonce']) && wp_verify_nonce($_POST['rotify_comment_nonce'], 'rotify_comment_submit')) {
        if ($can_submit) {
            ...
            if ($post_id) {
                ...
                echo '<p class="rotify-success rotify-submit-success">نظر شما با موفقیت ارسال شد و در انتظار تأیید است.</p>';
            }
        } else {
